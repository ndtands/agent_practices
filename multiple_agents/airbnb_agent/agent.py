...
class AirbnbAgent:
    """A specialized assistant for Airbnb accommodations using MCP tools.

    Attributes:
        model: The language model used for processing queries.
        mcp_tools (List[Any]): List of Model Context Protocol (MCP) tools for airbnb.
    ...
    def __init__(self, mcp_tools: List[Any]):
        """Initialize the AirbnbAgent with necessary dependencies.

        Args:
            mcp_tools: List of MCP tools for airbnb data retrieval.
        ...
        self.agent_runnable = self.agent_runnable.with_config(tags=["airbnb_agent"])

    async def ainvoke(self, query: str, session_id: str) -> Dict[str, Any]:
        """Process a airbnb query and return a structured response.

        Args:
            query: The user's airbnb-related query.
    ...
    async def astream(self, query: str, session_id: str) -> AsyncIterable[Dict[str, Any]]:
        """Stream airbnb query responses as they are generated.
